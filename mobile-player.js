    ...
    <script>
        ...
        async function init() {
            try {
                ...
                const { data: albums, error: albErr } = await supabaseClient.from('albums').select('*');
                if(albErr) throw albErr;
                window.ALBUMS = albums || [];

                const { data: tracks } = await supabaseClient.from('tracks').select('*').order('track_order', { ascending: true });
                tracks?.forEach(t => {
        ...
        window.viewAlbum = (id) => {
            ...
            const tracks = (window.TRACKS[id] || []).sort((a, b) => (a.track_order || 1) - (b.track_order || 1));
            const artist = window.ARTISTS[alb.artist_id] || { name: "TRM Records" };

            let html = `
            <div class="view-animate">
                <div style="padding: 40px 20px 20px; text-align:center;">
                    <span class="material-icons" onclick="viewHome()" style="float:left;">chevron_left</span>
                    <img src="${alb.art_url}" style="width:200px; border-radius:8px; box-shadow:0 10px 30px rgba(0,0,0,0.5);">
                    <h1 style="margin:15px 0 5px 0;">${alb.name}</h1>
                    <p style="color:var(--accent-green); margin:0;" onclick="viewArtist('${alb.artist_id}')">${artist.name}</p>
                </div>
                <div class="standard-padding">
                    ${tracks.map((t, i) => {
                        const isActive = currentTrackId === t.id;
                        const albumIds = t.album_id.split(',').map(s => s.trim());
                        const isPlayableAnywhere = albumIds.some(aid => {
                            const parentAlb = window.ALBUMS.find(a => a.id == aid);
                            return parentAlb && !parentAlb.coming_soon;
                        });

                        return `
                        <div class="track-row ${!isPlayableAnywhere ? 'disabled' : ''} ${isActive ? 'active-track' : ''}" 
                             style="display:flex; padding:15px 0; border-bottom:1px solid #111;" 
                             onclick="playTrack('${t.id}', true, '${alb.id}')">
                            <span style="width:30px; color:${isActive ? 'var(--accent-green)' : '#555'};">${isActive ? '●' : (t.track_order || 1)}</span>
                            <div style="flex:1;">
                                <div style="font-weight:bold;">${!isPlayableAnywhere ? 'Track ' + (t.track_order || i+1) : t.title}</div>
        ...
        window.playTrack = (trackId, autoPlay = true, explicitAlbumId = null) => {
            ...
            const albumIds = track.album_id.split(',').map(s => s.trim());
            const playableAlbum = window.ALBUMS.find(a => albumIds.includes(a.id) && !a.coming_soon);
